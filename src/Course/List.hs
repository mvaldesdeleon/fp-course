...
filter ::
  ...
  -> List a
filter _ Nil       = Nil
filter p (x :. xs) = ifThenElse (p x) (x :. rest) rest
    where
        rest = filter p xs
...
seqOptional (a :. as) = case a of
    Empty  -> Empty
    Full a -> seqOptional as P.>>= \as -> P.return (a :. as)
...
find ::
  ...
  -> Optional a
find _ Nil       = Empty
find p (a :. as) = ifThenElse (p a) (Full a) (find p as)
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 as = ln as 0
    where
        next as n = case as of
            Nil       -> False
            (a :. as) -> ln as (n + 1)
        ln as n = ifThenElse (n > 4) True (next as n)
